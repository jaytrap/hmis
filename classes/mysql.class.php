...
function gradeScoreUpgraded($score,$gradingSys){

        $score = round(doubleval($score));
        $gsid = $gradingSys;

        $this->Query("SELECT * FROM grading_system_desc WHERE grading_system_id = $gsid ORDER BY upper_limit DESC");

         while(!$this->EndOfSeek()){ $pairow = $this->Row();
             $upper = $pairow->upper_limit;
             $lower = $pairow->lower_limit;
              if($score<=$upper && $score>=$lower){

                  return $pairow->letter_grade;
    ...
    function gradeScoreUpdatedManual($score,$gradingSys){
        ...
        if($gradingSys==1){
            ...
            if($score<=100 && $score>=70){
                ...
                return "C";
            }elseif($score<=39 && $score>=35){
                return "D";
            }elseif($score<=34 && $score>=0){
                return "F";
            }
        }
...
function getCourse($id){
	//$prog = new MySQL();
	$whereArray['id'] = $id;
	$row_cous = $this->QuerySingleRowArray(MySQL::BuildSQLSelect("courses",$whereArray));
	return $row_cous['code'];	
}
...
function getCredit($id){
	...
	$whereArray['id'] = $id;
	$row_cous = $this->QuerySingleRowArray(MySQL::BuildSQLSelect("courses",$whereArray));
	return $row_cous['credit'];	
}


function getGrade($courseId, $indexNumber){
	$sql = "SELECT gb_main.totalscore FROM gb_main INNER JOIN gb_header ON gb_main.gb_header_id = gb_header.id WHERE 
	gb_main.indexnumber = '$indexNumber' AND gb_header.courseid = '$courseId'";
	$row_grade = $this->QuerySingleRowArray($sql);
	return $row_grade['totalscore'];	
}
...
function mean($aValues){return $fMean = array_sum($aValues) / count($aValues);}

 function getActiveAcademicYear(){

        $this->Query("SELECT * FROM main_academic_years WHERE status='Active'");
        $myacrow = $this->Row();
        $acid = $myacrow->id;
        return $acid;
...
function formatAcademicYear($id){

        $this->Query("SELECT * FROM main_academic_years WHERE id = $id");
        $frow = $this->Row();
        $facy = $frow->full_academic_year;
        return $facy;
...
function showFaculty($id){
        $fid = $id;
        $this->Query("SELECT * FROM faculties WHERE id = $fid");
        $frow = $this->Row();
        $facname = $frow->name;
